...
| activeSection v-if → router-view | ⬜ 待执行 | vue-router 已装、死 Dashboard.vue 已删、路由骨架就绪 |
| SSH/VNC 测试（2 个 skipped） | ⬜ 待执行 | RemotePanel 自管理 WebSocket 生命周期，需针对 RemotePanel 单独写测试（而非全局 App 级测试） |

### 下次继续点

1. **拆分 api/platform.ts**（最大技术债，1275 行，拆为 domain 文件后用 re-export 保持导入不变）
2. **max-lines warn → error**（api 拆分后阻塞解除）
3. **activeSection v-if → router-view**（最终一步：所有 v-if 切换换成 `<router-view>` + 12 个路由视图）
