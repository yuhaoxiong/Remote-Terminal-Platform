...
项目和功能的 `name` 支持中文。机器使用的 `code` 仅允许小写字母、数字和连字符；创建时可以省略，平台会分别生成 `project-<随机标识>` 或 `function-<随机标识>`。
...
后端接口为后续小批量部署预留最多 5 台设备；当前项目页面只允许选择一台。生成前会阻断以下情况：设备不属于该项目、设备不在线、初始化未就绪、实际与期望硬件规格不一致、SSH 配置不可用、项目未配置已发布版本、缺少对应硬件变体或制品文件丢失。

计划状态与确认规则：

- `ready`：预检完成，可在 24 小时内人工确认；
- `stale`：设备、项目功能、配置、版本覆盖、硬件变体或制品状态已变化，必须重新生成；
...
```json
{
  "release_id": 12,
  "reason": "单设备现场验证",
  "expires_at": "2026-07-24T12:00:00+08:00"
}
```

覆盖版本必须属于项目已启用的功能、处于已发布状态且到期时间晚于当前时间；计划会优先选择有效覆盖，并把来源、原因和到期时间写入预检快照。
