...
- [ ] 真实 API 可修改、释放 SSH/VNC 端口。
- [ ] 端口冲突不会产生部分提交。
- [ ] 前后端类型均声明端口字段。
- [ ] 新增真实后端集成测试，不只依赖前端 mock。
- [ ] CI 失败时部署不会启动。
- [ ] 部署绑定确定 Git SHA。
- [ ] 权威 `deploy.sh` 已进入版本控制。
- [ ] README 和部署文档默认值与实现一致。
...
- [ ] 设备端口编辑、清空、冲突和回滚均有真实 API 测试。
- [ ] 前后端 DTO 与后端 schema 一致。
- [ ] 失败 CI 不能触发生产部署。
- [ ] 部署产物绑定确定 Git SHA，支持记录和回退。
...
批准后从阶段 A 开始，只处理设备端口契约和发布安全。阶段 A 全部验收通过并提交后，才能进入阶段 B。若只计划一个短迭代，最小可交付范围为：
