...
“项目与功能”页面现可为单台在线且初始化就绪的设备生成部署预览，冻结项目全部功能的版本、硬件变体、配置和制品哈希。管理员先确认计划，再单独确认“开始执行”，平台才会通过 SSH 调用设备端 `edge-deploy`。设备使用短时令牌经 HTTPS 下载制品，校验 SHA-256 后依次运行 `preflight/install/configure/start/healthcheck`；生产设备失败后回滚，测试设备保留现场。计划有效期为 24 小时，相关状态变化后会失效；重复确认和重复启动均不会创建重复部署。
